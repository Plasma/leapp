...
export class SessionCardComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.awsRegions = await this.workspaceService.getAwsRegions();
    ...
    // eslint-disable-next-line @typescript-eslint/naming-convention
    this.selectedProfile = { Id: this.getProfileId(this.session), Name: await this.workspaceService.getProfileName(this.getProfileId(this.session)) };
  }
  ...
  async changeRegion() {
    if (this.selectedDefaultRegion) {
      ...
      await this.sessionService.update(this.session.sessionId, this.session);
  ...
  async changeProfile() {
    if (this.selectedProfile) {
      ...
      if(!await this.workspaceService.getProfileName(this.selectedProfile.Id)) {
        await this.workspaceService.addProfile(this.selectedProfile.Name);
      }
      ...
      (this.session as any).profileId = await this.workspaceService.getProfileId(this.selectedProfile.Name);
      await this.sessionService.update(this.session.sessionId, this.session);

      // === Now the only way to force refresh of pipes ======
      await this.sessionService.start(this.session.sessionId);
      await this.sessionService.stop(this.session.sessionId);
      // =====================================================
